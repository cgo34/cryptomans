...
class WaveFileReader {
    ...
    public static function ReadFile($filename, $readdata = false) {
        ...
        if (is_readable($filename)) {
            ...
            echo '<br>$totalhdrssize : '.$totalhdrssize;
            echo '<br>$filesize : '.$filesize;
            echo '<br>$data[\'size\'] : '.$data['size'];
            echo '<br>';
            echo (1024 - $totalhdrssize);
            echo '<br>';
            echo '<br>$readdata : '. $readdata;

            if ( ($filesize - $totalhdrssize) == $data['size']){
                echo '<br>good';
                echo '<br>';
                $data['data'] = fread($handle, 100000 - $totalhdrssize);
                $test = self::readWord($data['data']);
                print_r($test);
                echo '<br>';
                $test = self::readWord($data['data']);
                print_r($test);
